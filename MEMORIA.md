...
- ✅ **Árbol de habilidades VISUAL estilo D4** (HECHO 2026‑06‑16): `renderSkills` dibuja un **grafo de nodos conectados por líneas** (`#skill-tree.sk-graph`): raíz = **Básico** (generador) → tronco → **bus** horizontal → 4 columnas (una por core) → nodo **core** (cuadrado) → conector → **Mejora** (redondo) → conector → fila de **2 Aspectos** (redondos, excluyentes). Líneas con CSS (`.sk-trunk/.sk-cols::before/.sk-col::before/.sk-conn`). Tocar un core abre `skillDetailPopup` (subir rango + engarces de soporte); los nodos Mejora/Aspecto se asignan al tocarlos (`allocateSkillMod`). Estados visuales: learned/avail/owned/locked/tier‑locked.
