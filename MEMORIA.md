...
- ✅ **Estética de árbol orgánico (zig-zag + ramas SVG)** (HECHO 2026‑06‑16): el árbol se dibuja como un **árbol descendente en ZIG-ZAG**: los rombos (nodos) se desfasan a izq/der alternando (`marginLeft/Right`), un **SVG** (`.sk-links`, `drawSkillLinks`) traza el **espinazo** diagonal entre rombos (dorado) y las **ramas** curvas (Bézier) de cada rombo a sus habilidades; cada habilidad mantiene sus 3 mini-ramas de pasivos como twigs. Coordenadas por layout (`offsetLeft/Top` acumulado por la cadena `offsetParent`), así el SVG se escala/desplaza junto al grafo con el `transform`. El rombo va centrado arriba de sus habilidades (`.sk-knode-head` en columna).
