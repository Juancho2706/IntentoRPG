...
for (const c of CLASSES) {
  ...
  for (const m of list) {
    ...
    if (m.nodes.length !== 6) throw new Error(`${m.id} debe tener 6 nodos`);
...
console.log('Integridad de datos: 3×3 maestrías, 6 nodos c/u, 1 capstone, todos con efecto ✓');
...
if (p.mastery.points !== 5) throw new Error('no descontó el punto');
...
// capstone bloqueado hasta gastar 5
g.allocateMasteryNode('b_cap');
if (p.mastery.nodes['b_cap']) throw new Error('capstone no debe asignarse con <5 gastados');
// asigna el resto de no-capstone (b_as, b_ls req0; b_n1, b_n2 req3)
for (const id of ['b_as', 'b_ls', 'b_n1', 'b_n2']) g.allocateMasteryNode(id);
if (g.masterySpent() !== 5) throw new Error('debería haber 5 nodos asignados, hay ' + g.masterySpent());
g.allocateMasteryNode('b_cap'); // ahora sí (req 5)
if (!p.mastery.nodes['b_cap']) throw new Error('capstone debería asignarse con 5 gastados');
...
console.log('Gating por req: capstone solo con 5 nodos; poderes registrados en recompute ✓');
